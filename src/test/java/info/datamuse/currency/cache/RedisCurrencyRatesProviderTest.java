...
import info.datamuse.currency.cache.redis.JedisConnectionFactory.PooledJedisConnectionFactory;
import info.datamuse.currency.cache.redis.RedisCurrencyRatesProvider;
...
import static info.datamuse.currency.cache.redis.RedisCurrencyRatesProvider.DEFAULT_REDIS_KEY_PREFIX;
...
final class RedisCurrencyRatesProviderTest extends AbstractCachingCurrencyRatesProviderTest {
    ...
    private static final PooledJedisConnectionFactory jedisConnectionFactory = new PooledJedisConnectionFactory(
    ...
    protected AbstractCachingCurrencyRatesProvider getCachingCurrencyRatesProvider(final CurrencyRatesProvider apiProvider) {
        return new RedisCurrencyRatesProvider(jedisConnectionFactory, apiProvider);
    }
    ...
    void getExchangeRateWithAutoUpdate() throws InterruptedException {
        final RedisCurrencyRatesProvider redisProviderCurrencyConverter =
                new RedisCurrencyRatesProvider(jedisConnectionFactory, new FreeCurrencyConverterApiComProvider(), DEFAULT_REDIS_KEY_PREFIX,true);
        ...
        final RedisCurrencyRatesProvider redisProviderCurrencyConverter1 =
                new RedisCurrencyRatesProvider(jedisConnectionFactory, new FreeCurrencyConverterApiComProvider(), "currency/exchange/rates",true);
